...
class DotGridAnimation {
  constructor(config = {}) {
    ...
    this.config = {
      ...
      lowOpacityPercentage: config.lowOpacityPercentage || 0.0, // Percentage of dots with opacity 0.1
      zeroOpacityPercentage: config.zeroOpacityPercentage || 0.9, // Percentage of dots with opacity 0
      ...
      // Appearance
      baseColorVar: config.baseColorVar || "--color-element-highlight", // CSS variable for color
      fallbackColor: config.fallbackColor || "#DEE920", // Fallback if CSS var not found
  ...
  init() {
    // Find all elements with the data-extraction-highlight attribute
    const extractionElements = document.querySelectorAll("[data-extraction-highlight]");
